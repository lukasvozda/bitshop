...
<div class="dropdown w-full">
    <div class="relative max-w-xl">
        <input
            ...
            class="peer mt-1 block w-full rounded-md border-gray-300 shadow-sm disabled:bg-gray-200 focus:text-gray-600 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm {isValidField($fieldItem.invalid)}"
        >
        <div class="absolute right-0 inset-y-0 pr-3">
            <div class="pt-3"><ChevronDownIcon size="17"></ChevronDownIcon></div>
        </div>
        {#if filteredItems.length > 0}
            <ul tabindex="0" class="z-40 absolute dropdown-content border-gray-500 menu p-2 shadow bg-white rounded-box w-full invisible peer-focus:visible hover:visible">
        ...
        {#if $fieldItem.invalid }
            <span class="text-red-500 text-sm ml-1 mt-1">
                You must pick an item from the list
            </span>
        {/if}
    </div>
</div>
