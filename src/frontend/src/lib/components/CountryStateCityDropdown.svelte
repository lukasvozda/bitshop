<script>
    ...
    import { mustBeInList, mustHaveSpecifiedName } from "../utils/validators/index.ts";
    import { country, county, city } from "../../stores/shoppingCart/index.ts";
    ...
    let countries = Country.getAllCountries().map((country) => ({
        name: country.name, isoCode: country.isoCode, flag: country.flag
      })
    );
    ...
    const getCountryRegions = (country) => {
      if (country.valid) {
        return State.getStatesOfCountry(country.value.isoCode).map((state) => ({
            name: state.name, isoCode: state.isoCode
          })
        );
      } else {
        return [];
    ...
    const getCities = (country, region) => {
      if (country.valid) {
        if (region.valid && region.isoCode) {
          return City
            .getCitiesOfState(country.value.isoCode, region.value.isoCode)
            .map((city) => ({
              name: city.name
            })
          );
          ...
          return City
            .getCitiesOfCountry(country.value.isoCode)
            .map((city) => ({
              name: city.name
            })
          );
