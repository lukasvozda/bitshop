...
module {
	...
	// let publicPrefix : Nat32 = 0x043587cf;
	...
	public func parse(bip32Key : Text, _parentPubKey : ?ParentPublicKey) : ?ExtendedPublicKey {
		switch (Base58Check.decode(bip32Key)) {
			case (?b58Decoded) {
				...
				Debug.print(debug_show ("depth", depth));
				...
				if (b58Decoded[45] != 0x02 and b58Decoded[45] != 0x03) {
					Debug.print(debug_show ("Public key must start with 0x02 or 0x03.", b58Decoded[45], b58Decoded[45]));
					return null;
				};
				...
				return switch (Affine.fromBytes(keyData, curve)) {
					case (null) {
						Debug.print(debug_show ("Affine.fromBytes(keyData, curve) je null"));
						return null;
					};
					case (?parsedPoint) {
						if (not Affine.isOnCurve(parsedPoint)) {
							Debug.print(debug_show ("point not on curve"));
							return null;
						};
						?ExtendedPublicKey(keyData, chaincode, depth, index, fingerprint, parentPubKey);
					};
				};
			};
			case (null) {
				// Parsing as b58 string failed.
				Debug.print(debug_show ("Parsing as b58 string failed."));
				return null;
			};
		};
	};
	...
	public class ExtendedPublicKey(
		...
		public func serializeNonChild() : Text {
			let result = Array.init<Nat8>(78, 0);
