...
module {
  ...
  public func build_transaction(
    ...
    var total_fee : Nat = 0;
    loop {
      let transaction = Utils.get_ok_except(Bitcoin.buildTransaction(2, own_utxos, [(#p2pkh dst_address, amount)], #p2pkh own_address, Nat64.fromNat(total_fee)), "Error building transaction.");
  ...
  public func sign_transaction(
    ...
    switch (Address.scriptPubKey(#p2pkh own_address)) {
      ...
      case (#err msg) Debug.trap("This example supports signing p2pkh addresses only.");
    };
